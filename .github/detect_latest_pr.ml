...
let log fmt =
  Format.kasprintf
    (fun s -> if config.verbose then Printf.eprintf "%s\n%!" s)
    fmt
...
let () =
  Arg.parse
    [
      ...
      ("-template", Arg.String (fun s -> config.template_dir <- s), " ");
      ("-v", Arg.Unit (fun () -> config.verbose <- true), " verbose logging");
    ]
    (fun s -> config.user_branch <- s)
    "TODO: help";
  if config.user = "" then (
    eprintfn "User is empty";
    exit 1);
  if config.repo = "" then (
    eprintfn "Repo is empty";
    exit 1)
...
let () =
  ...
  if not (List.mem "upstream" @@ Str.split (Str.regexp "\n") s) then
    let _ =
      commandf "git remote add upstream https://github.com/%s/%s.git"
        config.user config.repo
    in
...
let calculate_common_subdir files =
  ...
  in
  let set =
    List.fold_right (fun x acc -> SS.add (get_top_dir x) acc) files SS.empty
  in
...
let merge_base =
  ...
  | PR -> (
      let s =
        get_output "git merge-base upstream/master %s" (* user_branch *) "HEAD"
      in
...
let the_HEAD =
  match config.mode with Master -> "HEAD" | PR -> config.user_branch
...
let () =
  let s =
    get_output "git diff-tree %s..%s  | rev | cut -f 1 | rev" merge_base
      the_HEAD
  in
  log "%S " s;
  let changed_files =
