...
let rec free_vars_cexpr : cexpr -> (string, String.comparator_witness) S.t = function
  ...
  | CFun (arg, body) ->
      S.remove (free_vars_aexpr body) arg
  ...
  | ALet (rec_flag, name, rhs, body) ->
      (match rhs with
       | CFun (arg, f_body) ->
           ...
           let helper_item =
             ALet (Nonrecursive, helper_name, helper_fun_cexpr,
                   ALet ( (match rec_flag with Recursive -> Recursive | Nonrecursive -> Nonrecursive)
                        , name
                        , closure_cexpr
                        , new_let_body))
           in
...
and cc_cexpr (cexpr : cexpr) : cexpr CCState.t =
  ...
  | CFun _ ->
      error "unreachable: cc_cexpr should not encounter CFun"
;;
...
let cc_str_item (item : astructure_item) : astructure_item list CCState.t =
  ...
  | AStr_value (rec_flag, name, (ACE (CFun (arg, f_body)) as func_aexpr)) ->
      ...
      let captured_list = S.to_list (S.inter fvs st_before.bound) in
      let* helper_name = fresh_name name in
      let inner_bound_vars =
        S.union
          (S.of_list (module String) (arg :: captured_list))
      ...
      let closure_item =
        AStr_value
          ( (match rec_flag with Recursive -> Recursive | Nonrecursive -> Nonrecursive)
          , name
          , ACE closure_cexpr )
      in
      let* st_after = get in
