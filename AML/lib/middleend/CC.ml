    ...
    (match rhs with
     | CFun (arg, f_body) ->
       ...
       let fvs_for_env =
         ...
         | Nonrecursive -> fvs
       in
       let captured_list = S.to_list (S.inter fvs_for_env st.bound) in
       let* helper_name = fresh_name name in
       let inner_bound_vars =
         S.union
           (S.of_list (module String) (arg :: captured_list))
           (match rec_flag with
            | Recursive -> S.singleton (module String) name
...
let cc_str_item (item : astructure_item) : astructure_item list CCState.t =
  ...
  | AStr_value (rec_flag, name, (ACE (CFun (arg, f_body)) as func_aexpr)) ->
    ...
    let captured_list = Set.to_list (Set.inter fvs st_before.bound) in
    let* helper_name = fresh_name name in
    let helper_params =
      match rec_flag with
      | Recursive -> name :: (captured_list @ [ arg ])
      | Nonrecursive -> captured_list @ [ arg ]
    in
    let inner_bound =
      Set.union st_before.bound (Set.of_list (module String) helper_params)
    in
    let* new_f_body = with_new_scope inner_bound st_before.subst (cc_aexpr f_body) in
    let rec mk_fun params body =
      match params with
      | [] -> body
      | p :: ps -> ACE (CFun (p, mk_fun ps body))
    in
    let helper_fun_aexpr = mk_fun helper_params new_f_body in
    let closure_args =
      match rec_flag with
      | Recursive -> ImmId name :: List.map captured_list ~f:(fun id -> ImmId id)
      | Nonrecursive -> List.map captured_list ~f:(fun id -> ImmId id)
    in
    let closure_item =
      match rec_flag with
      | Nonrecursive ->
        let clos = CApp (ImmId helper_name, closure_args) in
        AStr_value (Nonrecursive, name, ACE clos)
      | Recursive ->
        let rhs =
          ACE (CFun (arg, ACE (CApp (ImmId helper_name, closure_args @ [ ImmId arg ]))))
        in
        AStr_value (Recursive, name, rhs)
    in
    let helper_item = AStr_value (Nonrecursive, helper_name, helper_fun_aexpr) in
    let* st_after = get in
    let new_top = Set.add (Set.add st_before.bound name) helper_name in
    let* () = put { temps = st_after.temps; bound = new_top; subst = st_before.subst } in
    return [ helper_item; closure_item ]
  | AStr_value (rec_flag, name, aexpr) ->
    let* st_before = get in
    let new_bound_for_body =
      match rec_flag with
      | Recursive -> Set.add st_before.bound name
      | Nonrecursive -> st_before.bound
    in
    let* new_aexpr = with_new_scope new_bound_for_body st_before.subst (cc_aexpr aexpr) in
    let* st_after = get in
    let new_top = Set.add st_before.bound name in
    let* () = put { temps = st_after.temps; bound = new_top; subst = st_before.subst } in
