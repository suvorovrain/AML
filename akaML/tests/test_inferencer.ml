...
let run str =
  ...
  | Ok ast ->
    (match run_inferencer env_with_print_funs ast with
     | Ok (_, out_list) ->
       List.iter
         (function
...
let%expect_test "type check sequence" =
  run
    {|
  let a = print_int 21; 22
  |};
  [%expect
    {|
  val a : int
...
let%expect_test "type check operator redefinition" =
  run
    {|
  let foo1 x y = x + y
  let (+) = (&&)
