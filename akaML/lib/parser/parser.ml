...
let parse_operator op_list =
  choice (List.map ~f:(fun opr -> token opr *> return opr) op_list)
;;

let parse_op_ident = ws *> char '(' *> ws *> parse_operator bin_op_list <* ws <* char ')'
let parse_ident = parse_val_ident <|> parse_op_ident
...
let bin_op chain1 parse_exp parse_fun_op =
  chain1
    parse_exp
    (parse_fun_op
     >>| fun opr exp1 exp2 -> Exp_apply (Exp_apply (Exp_ident opr, exp1), exp2))
;;
